...
// (566:0) {#if showBotUI && !showUnfriendlyError}
function create_if_block(ctx) {
	let div6;
	let div1;
	let div0;
	let raw0_value = marked_1(/*frameIntroduction*/ ctx[8]) + "";
	let t0;
	let t1;
	let div5;
	let div2;
	let p;
	let raw1_value = marked_1(/*completedRounds*/ ctx[1][/*completedRounds*/ ctx[1].length - 1].slot.ask) + "";
	let t2;
	let div4;
	let div3;
	...
	return {
		...
		p(ctx, dirty) {
			if ((!current || dirty[0] & /*frameIntroduction*/ 256) && raw0_value !== (raw0_value = marked_1(/*frameIntroduction*/ ctx[8]) + "")) div0.innerHTML = raw0_value;
			...
			if ((!current || dirty[0] & /*completedRounds*/ 2) && raw1_value !== (raw1_value = marked_1(/*completedRounds*/ ctx[1][/*completedRounds*/ ctx[1].length - 1].slot.ask) + "")) p.innerHTML = raw1_value;			let previous_block_index = current_block_type_index;
...
// (597:10) {#if userReplyValues.length > 0}
...
// (586:6) {#each completedRounds.slice(0, -1) as { slot, userReplyValues }
function create_each_block_2(ctx) {
	let div;
	let p;
	let raw_value = marked_1(/*slot*/ ctx[42].ask) + "";
	let t0;
	let t1;
	...
	return {
		...
		p(ctx, dirty) {
			if (dirty[0] & /*completedRounds*/ 2 && raw_value !== (raw_value = marked_1(/*slot*/ ctx[42].ask) + "")) p.innerHTML = raw_value;
...
// (639:10) {#if replyType === slotTypeEnum.diagnostic || (replyType === slotTypeEnum.single && replyOptions[0] === BUILT_IN_REPLIES.done[0])}
...
// (672:18) {#each replyOptions as userReplyValue, userReplyIndex}
...
// (642:16) {#each adaptRepliesToText(replyOptions) as userReplyValue, userReplyIndex}
